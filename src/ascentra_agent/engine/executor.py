...
from ascentra_agent.contracts.specs import CutSpec, DimensionSpec, SegmentSpec
from ascentra_agent.engine.masks import build_mask
from ascentra_agent.engine.metrics import (
    compute_bottom2box,
    compute_frequency,
    compute_mean,
...
from ascentra_agent.engine.tables import TableResult, add_base_size_warnings
...
class Executor:
    ...
    def materialize_segments(self) -> dict[str, int]:
        ...
        Returns:
            Dict mapping segment_id to base size (count of matching rows)
        """
        base_sizes = {}

        for segment_id, segment in self.segments_by_id.items():
            mask = build_mask(self.df, segment.definition, self.questions_by_id)
            self._segment_masks[segment_id] = mask
            base_sizes[segment_id] = int(mask.sum())
    ...
    def _compute_metric_with_dimensions(
        ...
        if dim.kind == "question":
            ...
            # Split into in-segment and out-of-segment
            groups = {
                f"{dim.id}": df[segment_mask & df.index.isin(df.index)],
                f"Not {dim.id}": df[~segment_mask & df.index.isin(df.index)],
            }
